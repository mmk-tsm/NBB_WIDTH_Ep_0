...
/* STATIC VARIABLES             */
/* ---------------------------- */
EIPS_TIMER_STRUCT EIPS_Timers[EIPS_USER_MAX_NUM_TIMERS];
uint16 EIPS_UsedTimerIx[EIPS_USER_MAX_NUM_TIMERS];
...
void eips_timer_init (uint8 init_type)
{
    ...
    for(i=0; i<EIPS_USER_MAX_NUM_TIMERS; i++)
    {
        eips_timer_free(i);
    }
...
uint16 eips_timer_new (void)
{
    ...
    if(EIPS_NumTimerIxUsed == EIPS_USER_MAX_NUM_TIMERS)
        return(EIPS_TIMER_NULL);

    /* search through all timers for a free one */
    for(i=1; i<EIPS_USER_MAX_NUM_TIMERS; i++)
    {
...
void eips_timer_free (uint16 timer_num)
{
    ...
    if(timer_num >= EIPS_USER_MAX_NUM_TIMERS)
        return;
...
{
   /* validate timer_num is in range */
   if(!timer_num || timer_num >= EIPS_USER_MAX_NUM_TIMERS)
      return;
...
void EIPS_UpdateTimerIxTable (void)
{
    ...
    for(i=0, EIPS_NumTimerIxUsed = 0; i<EIPS_USER_MAX_NUM_TIMERS; i++)
    {
