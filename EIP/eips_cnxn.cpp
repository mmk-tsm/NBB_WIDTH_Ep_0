...
#if EIPS_PRINTALL > 2
    void eips_cnxn_print_fwdopen (EIPS_CONNECTION_DATA_STRUCT *temp_cnxn);
#endif
...
#if EIPS_PRINTALL > 2
    void eips_cnxn_print_fwdclose (EIPS_CONNECTION_DATA_STRUCT *temp_cnxn);
#endif

void eips_cnxn_parse_fwdclose (EIPS_CONNECTION_DATA_STRUCT *temp_cnxn, CPF_MESSAGE_STRUCT *cpf);
void local_print_cnxn_status (char *);
...
static uint8 eips_cnxn_timeout_flag;
...
local_print_cnxn_status("eips_cnxn_init");
}
...
void eips_cnxn_emTimeout (uint16 timer_num)
{
    ...
    for(i=0; i<EIPS_CNXN_MAXTOTALCNXNS; i++)
    {
        ...
        {         
            /* inform the processing of the timeout */
            eips_cnxn_timeout();
...
/*                eips_usersys_fatalError ("eips_cnxn_EMTimeout", 1); */
                return;
            }
            ...
            if(!cnxn_used)
                eips_usersock_tcpClose(session->sock_id);
        }
   }

/*local_print_cnxn_status("eips_cnxn_emTimeout");*/
}
...
void eips_cnxn_structInit(EIPS_CONNECTION_DATA_STRUCT *cnxnptr)
{
    ...
    cnxnptr->O2T_RPI = 0;
    cnxnptr->O2T_API = 0;
    cnxnptr->O2T_connection_parameters = 0;
    cnxnptr->T2O_RPI = 0;
    cnxnptr->T2O_API = 0;
    cnxnptr->T2O_connection_parameters = 0;
...
void eips_cnxn_structCopy (EIPS_CONNECTION_DATA_STRUCT *dst_cnxnptr, EIPS_CONNECTION_DATA_STRUCT *src_cnxnptr)
{
    ...
    memcpy((uint8 *)&dst_cnxnptr->O2T_connection_parameters, (uint8 *)&src_cnxnptr->O2T_connection_parameters, sizeof(dst_cnxnptr->O2T_connection_parameters));
    memcpy((uint8 *)&dst_cnxnptr->T2O_RPI, (uint8 *)&src_cnxnptr->T2O_RPI, sizeof(dst_cnxnptr->T2O_RPI));
    memcpy((uint8 *)&dst_cnxnptr->T2O_connection_parameters, (uint8 *)&src_cnxnptr->T2O_connection_parameters, sizeof(dst_cnxnptr->T2O_connection_parameters));
...
(UINT16)  O_to_T Cnxn Params    SEE REQ / RSP PARAMS BELOW
(UINT32)  T_to_O RPI            T to O req packet rate in microsecs
(UINT16)  T_to_O Cnxn Params    SEE REQ / RSP PARAMS BELOW
...
/*  local_print_cnxn_status("eips_cnxn_fwdOpen beg"); */
    ...
    temp_cnxn.O2T_connection_parameters = rta_GetLitEndian16(cpf->emreq->user_struct.req_data + buf_ix);
    buf_ix += 2; /* advance the pointer */
    ...
    temp_cnxn.T2O_connection_parameters = rta_GetLitEndian16(cpf->emreq->user_struct.req_data + buf_ix);
    buf_ix += 2; /* advance the pointer */
    ...
    /* print the contents of the forward open */
#if EIPS_PRINTALL > 2
    eips_cnxn_print_fwdopen (&temp_cnxn);
#endif

    /* parse and process the forward open */
    eips_cnxn_parse_fwdopen (&temp_cnxn, cpf);

/*	eips_user_dbprint0("eips_cnxn_fwdOpen end"); */
}
...
void eips_cnxn_parse_fwdopen (EIPS_CONNECTION_DATA_STRUCT *temp_cnxn, CPF_MESSAGE_STRUCT *cpf)
{
    ...
    for(i=0; i<EIPS_USER_MAX_NUM_EM_CNXNS_PER_SESSION; i++)
    {
        ...
        if(session->CnxnPtrs[i] != NULLPTR)
        {
                ...
                (session->CnxnPtrs[i]->originator_serial_number == temp_cnxn->originator_serial_number))
            {
                ...
                if( ((temp_cnxn->T2O_connection_parameters & NTWKCNXNPRM_CNXNTYP_ANDVAL) == NTWKCNXNPRM_CNXNTYP_NULL) &&
                    ((temp_cnxn->O2T_connection_parameters & NTWKCNXNPRM_CNXNTYP_ANDVAL) == NTWKCNXNPRM_CNXNTYP_NULL))
                {
    ...
    temp16 = (uint16)(temp_cnxn->T2O_connection_parameters & NTWKCNXNPRM_CNXNTYP_ANDVAL);
    if(temp16 != NTWKCNXNPRM_CNXNTYP_P2P)
    {
        /* Error: "Invalid Connection Type" */
        cpf->emrsp->user_struct.gen_stat = ERR_CNXN_FAILURE;
        cpf->emrsp->user_struct.ext_stat_size = 1;
        cpf->emrsp->user_struct.ext_stat[0] = CMERR_BAD_CONN_TYPE;
        eips_cnxn_errorFwdOpen(temp_cnxn,cpf);
        return;
    }
    temp16 = (uint16)(temp_cnxn->O2T_connection_parameters & NTWKCNXNPRM_CNXNTYP_ANDVAL);
    if(temp16 != NTWKCNXNPRM_CNXNTYP_P2P)
    {
        /* Error: "Invalid Connection Type" */
        cpf->emrsp->user_struct.gen_stat = ERR_CNXN_FAILURE;
                    ...
                    /*  The minor revision is dependent on the major revision
                        ...
                        if(minor_rev)
                        {
                            ...
                            if( ((minor_rev != EIPS_MINOR_REV) && (compatibility == 0)) ||
                                ((minor_rev > EIPS_MINOR_REV)  && (compatibility == 1)))
                            {
                                /* Error: "The Revision Didn't Match" */
                                cpf->emrsp->user_struct.gen_stat = ERR_CNXN_FAILURE;
                                cpf->emrsp->user_struct.ext_stat_size = 1;
...
void eips_cnxn_validFwdOpen (EIPS_CONNECTION_DATA_STRUCT *temp_cnxn, CPF_MESSAGE_STRUCT *cpf)
{
    ...
    /* clear the timeout flag (on any successful open) */
    eips_cnxn_timeout_flag = 0;
...
local_print_cnxn_status("eips_cnxn_fwdClose beg");
    ...
    /* print the contents of the forward close */
#if EIPS_PRINTALL > 2
    eips_cnxn_print_fwdclose (&temp_cnxn);
#endif

    /* parse and process the forward close */
    eips_cnxn_parse_fwdclose (&temp_cnxn, cpf);

local_print_cnxn_status("eips_cnxn_fwdClose end");
}
...
/* ====================================================================
Function:   eips_cnxn_print_fwdopen
Parameters: connection data pointer
Returns:    N/A

This function prints the content of the connection pointer.
======================================================================= */
#if EIPS_PRINTALL > 2
void eips_cnxn_print_fwdopen (EIPS_CONNECTION_DATA_STRUCT *temp_cnxn)
{
    ...
    /* ************************* */
    eips_user_dbprint1(" FWD OPEN->O_to_T Cnxn ID:     %lu\r\n",    temp_cnxn->O2T_CID);
    eips_user_dbprint1(" FWD OPEN->T_to_O Cnxn ID:     %lu\r\n",    temp_cnxn->T2O_CID);
    eips_user_dbprint1(" FWD OPEN->Cnxn Serial #:      %u\r\n",     temp_cnxn->connection_serial_number);
    eips_user_dbprint1(" FWD OPEN->Orig Vendor ID:     %u\r\n",     temp_cnxn->vendor_ID);
    eips_user_dbprint1(" FWD OPEN->Orig Serial #:      %lu\r\n",    temp_cnxn->originator_serial_number);
    eips_user_dbprint1(" FWD OPEN->Cnxn Tmo Mult:      %d\r\n",     temp_cnxn->connection_timeout_multiplier);
    eips_user_dbprint1(" FWD OPEN->O_to_T RPI:         %ld\r\n",    temp_cnxn->O2T_RPI);
    eips_user_dbprint1(" FWD OPEN->T_to_O RPI:         %ld\r\n",    temp_cnxn->T2O_RPI);
    eips_user_dbprint0("\r\n"); /* print a blank line */

    eips_user_dbprint1(" FWD OPEN->Xport Type/Trigger: 0x%02x\r\n", temp_cnxn->xport_type_and_trigger);
    if(temp_cnxn->xport_type_and_trigger & 0x80)
        {eips_user_dbprint0("  transportClass_trigger: Server\r\n");}
    else
        {eips_user_dbprint0("  transportClass_trigger: Client\r\n");}

    if((temp_cnxn->xport_type_and_trigger & 0x70) == 0x00)
        {eips_user_dbprint0("  transportClass_trigger: Cyclic\r\n");}
    else if((temp_cnxn->xport_type_and_trigger & 0x70) == 0x10)
        {eips_user_dbprint0("  transportClass_trigger: COS\r\n");}
    else if((temp_cnxn->xport_type_and_trigger & 0x70) == 0x20)
        {eips_user_dbprint0("  transportClass_trigger: Application Object\r\n");}
    else
        {eips_user_dbprint0("  transportClass_trigger: Reserved\r\n");}
    eips_user_dbprint1("  transportClass_trigger: Transport Class %d\r\n",(temp_cnxn->xport_type_and_trigger & 0x0F));
    eips_user_dbprint0("\r\n"); /* print a blank line */

    eips_user_dbprint1(" FWD OPEN->Prior/Time_tick:    %d\r\n",     temp_cnxn->priority_and_tick);
    eips_user_dbprint1(" FWD OPEN->Time-out_ticks:     %d\r\n",     temp_cnxn->connection_time_out_ticks);
    eips_user_dbprint1("  Transaction Timeout: %ld\r\n",(EIPS_TIMETICKVALENUM[(temp_cnxn->priority_and_tick&0x0F)]*temp_cnxn->connection_time_out_ticks));
    eips_user_dbprint0("\r\n"); /* print a blank line */

    eips_user_dbprint1(" FWD OPEN->O_to_T Cnxn Param:  0x%04x\r\n", temp_cnxn->O2T_connection_parameters);
    /* O_to_T Network Connection Parameter "Redundant Owner" */
    if((temp_cnxn->O2T_connection_parameters & NTWKCNXNPRM_OWNER_ANDVAL) == NTWKCNXNPRM_OWNER_ONE)
        {eips_user_dbprint0("  O2T Owners: One\r\n");}
    else
        {eips_user_dbprint0("  O2T Owners: Many\r\n");}

    /* O_to_T Network Connection Parameter "Connection Type" */
    if((temp_cnxn->O2T_connection_parameters & NTWKCNXNPRM_CNXNTYP_ANDVAL) == NTWKCNXNPRM_CNXNTYP_NULL)
        {eips_user_dbprint0("  O2T Cnxn Type: Null\r\n");}
    else if((temp_cnxn->O2T_connection_parameters & NTWKCNXNPRM_CNXNTYP_ANDVAL) == NTWKCNXNPRM_CNXNTYP_MULTI)
        {eips_user_dbprint0("  O2T Cnxn Type: Multicast\r\n");}
    else if((temp_cnxn->O2T_connection_parameters & NTWKCNXNPRM_CNXNTYP_ANDVAL) == NTWKCNXNPRM_CNXNTYP_P2P)
        {eips_user_dbprint0("  O2T Cnxn Type: Point to Point\r\n");}
    else
        {eips_user_dbprint0("  O2T Cnxn Type: Reserved\r\n");}

    /* O_to_T Network Connection Parameter "Priority" */
    if((temp_cnxn->O2T_connection_parameters & NTWKCNXNPRM_PRIORITY_ANDVAL) == NTWKCNXNPRM_PRIORITY_LOW)
        {eips_user_dbprint0("  O2T Priority: Low\r\n");}
    else if((temp_cnxn->O2T_connection_parameters & NTWKCNXNPRM_PRIORITY_ANDVAL) == NTWKCNXNPRM_PRIORITY_HIGH)
        {eips_user_dbprint0("  O2T Priority: High\r\n");}
    else if((temp_cnxn->O2T_connection_parameters & NTWKCNXNPRM_PRIORITY_ANDVAL) == NTWKCNXNPRM_PRIORITY_SCHEDULE)
        {eips_user_dbprint0("  O2T Priority: Scheduled\r\n");}
    else
        {eips_user_dbprint0("  O2T Priority: Reserved\r\n");}
    /* O_to_T Network Connection Parameter "Fixed/Varible Size" */
    if((temp_cnxn->O2T_connection_parameters & NTWKCNXNPRM_FIXED_ANDVAL) == NTWKCNXNPRM_FIXED_SIZ)
        {eips_user_dbprint0("  O2T Size: Fixed\r\n");}
    else
        {eips_user_dbprint0("  O2T Size: Varible\r\n");}
    /* O_to_T Network Connection Parameter "Connection Size" */
    eips_user_dbprint1("  O2T Connection Size: %d\r\n",(temp_cnxn->O2T_connection_parameters & NTWKCNXNPRM_CNXN_SIZ_ANDVAL));
    eips_user_dbprint0("\r\n"); /* print a blank line */

    eips_user_dbprint1(" FWD OPEN->T_to_O Cnxn Param:  0x%04x\r\n", temp_cnxn->T2O_connection_parameters);
    /* T_to_O Network Connection Parameter "Redundant Owner" */
    if((temp_cnxn->T2O_connection_parameters & NTWKCNXNPRM_OWNER_ANDVAL) == NTWKCNXNPRM_OWNER_ONE)
        {eips_user_dbprint0("  T2O Owners: One\r\n");}
    else
        {eips_user_dbprint0("  T2O Owners: Many\r\n");}

    /* T_to_O Network Connection Parameter "Connection Type" */
    if((temp_cnxn->T2O_connection_parameters & NTWKCNXNPRM_CNXNTYP_ANDVAL) == NTWKCNXNPRM_CNXNTYP_NULL)
        {eips_user_dbprint0("  T2O Cnxn Type: Null\r\n");}
    else if((temp_cnxn->T2O_connection_parameters & NTWKCNXNPRM_CNXNTYP_ANDVAL) == NTWKCNXNPRM_CNXNTYP_MULTI)
        {eips_user_dbprint0("  T2O Cnxn Type: Multicast\r\n");}
    else if((temp_cnxn->T2O_connection_parameters & NTWKCNXNPRM_CNXNTYP_ANDVAL) == NTWKCNXNPRM_CNXNTYP_P2P)
        {eips_user_dbprint0("  T2O Cnxn Type: Point to Point\r\n");}
    else
        {eips_user_dbprint0("  T2O Cnxn Type: Reserved\r\n");}

    /* T_to_O Network Connection Parameter "Priority" */
    if((temp_cnxn->T2O_connection_parameters & NTWKCNXNPRM_PRIORITY_ANDVAL) == NTWKCNXNPRM_PRIORITY_LOW)
        {eips_user_dbprint0("  T2O Priority: Low\r\n");}
    else if((temp_cnxn->T2O_connection_parameters & NTWKCNXNPRM_PRIORITY_ANDVAL) == NTWKCNXNPRM_PRIORITY_HIGH)
        {eips_user_dbprint0("  T2O Priority: High\r\n");}
    else if((temp_cnxn->T2O_connection_parameters & NTWKCNXNPRM_PRIORITY_ANDVAL) == NTWKCNXNPRM_PRIORITY_SCHEDULE)
        {eips_user_dbprint0("  T2O Priority: Scheduled\r\n");}
    else
        {eips_user_dbprint0("  T2O Priority: Reserved\r\n");}

    /* T_to_O Network Connection Parameter "Fixed/Varible Size" */
    if((temp_cnxn->T2O_connection_parameters & NTWKCNXNPRM_FIXED_ANDVAL) == NTWKCNXNPRM_FIXED_SIZ)
        {eips_user_dbprint0("  T2O Size: Fixed\r\n");}
    else
        {eips_user_dbprint0("  T2O Size: Varible\r\n");}
   
    /* T_to_O Network Connection Parameter "Connection Size" */
    eips_user_dbprint1("  T2O Connection Size: %d\r\n",(temp_cnxn->T2O_connection_parameters & NTWKCNXNPRM_CNXN_SIZ_ANDVAL));
    eips_user_dbprint0("\r\n"); /* print a blank line */

    eips_user_dbprint1(" FWD OPEN->Path Size: %d\r\n",     temp_cnxn->connection_path_size);
    if(temp_cnxn->connection_path_size)
    {
        eips_user_dbprint0(" FWD OPEN->Path: ");
        for(i=0; i<temp_cnxn->connection_path_size; i++)
            {eips_user_dbprint1("0x%04x ", temp_cnxn->connection_path[i]);}
        eips_user_dbprint0("\r\n");
    }
    eips_user_dbprint0("\r\n"); /* print a blank line */
}
...
/* ====================================================================
Function:   eips_cnxn_print_fwdclose
Parameters: connection data pointer
Returns:    N/A
...
void eips_cnxn_print_fwdclose (EIPS_CONNECTION_DATA_STRUCT *temp_cnxn)
{
    ...
    /* Print the Connection Info */
    /* ************************* */
    eips_user_dbprint1(" FWD CLOSE->Prior/Time_tick:    %d\r\n",     temp_cnxn->priority_and_tick);
    eips_user_dbprint1(" FWD CLOSE->Time-out_ticks:     %d\r\n",     temp_cnxn->connection_time_out_ticks);
    eips_user_dbprint1("  Transaction Timeout: %ld\r\n",(EIPS_TIMETICKVALENUM[(temp_cnxn->priority_and_tick&0x0F)]*temp_cnxn->connection_time_out_ticks));
    eips_user_dbprint0("\r\n"); /* print a blank line */

    eips_user_dbprint1(" FWD CLOSE->Cnxn Serial #:      %u\r\n",     temp_cnxn->connection_serial_number);
    eips_user_dbprint1(" FWD CLOSE->Orig Vendor ID:     %u\r\n",     temp_cnxn->vendor_ID);
    eips_user_dbprint1(" FWD CLOSE->Orig Serial #:      %lu\r\n",    temp_cnxn->originator_serial_number);

    eips_user_dbprint1(" FWD CLOSE->Path Size: %d\r\n",     temp_cnxn->connection_path_size);
    if(temp_cnxn->connection_path_size)
    {
        eips_user_dbprint0(" FWD CLOSE->Path: ");
        for(i=0; i<temp_cnxn->connection_path_size; i++)
            {eips_user_dbprint1("0x%04x ", temp_cnxn->connection_path[i]);}
        eips_user_dbprint0("\r\n");
    }
...
/* ====================================================================
Function:   local_print_cnxn_status
Parameters: N/A
Returns:    N/A

This function prints which connections are open.
======================================================================= */
void local_print_cnxn_status (char *s)
{
    RTA_UNUSED_PARAM(s);
    ...
    eips_user_dbprint1("status(%s) ",s);
    for(i=0; i<EIPS_CNXN_MAXTOTALCNXNS; i++)
    {
        if(eips_CnxnStruct[i].state == EIPS_CNXN_STATE_FREE)
            eips_user_dbprint0(".");
        else
            eips_user_dbprint0("X");

        if(!((i+1)%10))
            eips_user_dbprint0(" ");
    }
    eips_user_dbprint1(" (%d cnxns)\r\n",EIPS_CNXN_MAXTOTALCNXNS);
#endif
}
