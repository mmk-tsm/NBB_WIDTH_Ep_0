...
static void     local_iomsg_KickStartMulticast (uint32);
...
void eips_iomsg_open (CPF_MESSAGE_STRUCT *cpf, EIPS_CONNECTION_DATA_STRUCT *temp_cnxn)
{
    ...
    uint8   config_data[450];
    uint8   config_data_size;
    uint8   response_code;
    uint16  additional_response_code;
    
    EIPS_IOCNXN_STRUCT *iomsg_struct_ptr;
    EIPS_IOCNXN_STRUCT *owning_iomsg_struct_ptr;
    ...
    if( (temp16 != NTWKCNXNPRM_CNXNTYP_P2P) && (temp16 != NTWKCNXNPRM_CNXNTYP_MULTI) )
    {
        ...
        local_iomsg_buildError(cpf, ERR_CNXN_FAILURE, 1, CMERR_BAD_CONN_TYPE);
    ...
    */
        ...
        switch (path_seg)
        {
            ...
            case DATASEG_SIMPLE:
                ...
                for(j=0; j<config_data_size; j++, i++)
                {
                    ...
                    else
                    {
                        ...
                        local_iomsg_buildError(cpf, ERR_CNXN_FAILURE, 1, CMERR_BAD_SEGMENT);
                        eips_cnxn_errorFwdOpen(temp_cnxn,cpf);
                        return;
                    }
                }
	        ...
	        case LOGSEG_ELECKEY:
	            ...
	            if(path_val == ELECKEY_FORMAT)
	            {
	                ...
	                compatibility = (uint8)((major_rev & 0x80) >> 7);
		                ...
		                ((prod_code) && (prod_code != eips_IDObj.Inst.ProductCode)) )
	                {
                        ...
                        local_iomsg_buildError(cpf, ERR_CNXN_FAILURE, 1, CMERR_BAD_VENDOR_PRODUCT);
		                eips_cnxn_errorFwdOpen(temp_cnxn,cpf);
		                return;
	                }
	                ...
	                if((dev_type) && (dev_type != eips_IDObj.Inst.DeviceType))
	                {
                        ...
                        local_iomsg_buildError(cpf, ERR_CNXN_FAILURE, 1, CMERR_BAD_DEVICE_TYPE);
		                eips_cnxn_errorFwdOpen(temp_cnxn,cpf);
		                return;
	                }
	                ...
	                if(major_rev)
	                {
		                ...
		                if(major_rev != EIPS_MAJOR_REV)
		                {
                            ...
                            local_iomsg_buildError(cpf, ERR_CNXN_FAILURE, 1, CMERR_BAD_REVISION);
		                    eips_cnxn_errorFwdOpen(temp_cnxn,cpf);
		                    return;
		                }
    ...
    /*  we need to validate the class, (cfg inst if present),
        cnxnpt1 (O_to_T) and cnxnpt2 (T_to_O) */
    if( (class_found   == FALSE) || (class_id != CLASS_ASSEMBLY) ||
        (cnxnpt1_found == FALSE) || (eips_cpf_o2tCnxnPtValid(cnxnpt_o2t) == FALSE) ||
        (cnxnpt2_found == FALSE) || (eips_cpf_t2oCnxnPtValid(cnxnpt_t2o) == FALSE) )
    {
        /* Error: "Invalid Segment Type or Value in Path" */
        local_iomsg_buildError(cpf, ERR_CNXN_FAILURE, 1, CMERR_BAD_SEGMENT);
        eips_cnxn_errorFwdOpen(temp_cnxn,cpf);
    ...
    if(cfg_inst_found == TRUE)
    {
        ...
        response_code = 0;
        additional_response_code = 0;

        /* validate the configuration assembly */
        if(eips_userobj_cfg_asm_process((uint16)cfg_inst_id, config_data, (uint16)(config_data_size*2), &response_code, &additional_response_code) != SUCCESS)
        {
            /* we have an error code */
            if(response_code != 0)
            {
                /* we have an additional error code */
                if(additional_response_code != 0)
                {
        ...
        (temp_cnxn->xport_type_and_trigger == EIPS_XPORTCLASS_APP_NOSEQ) )
    {
        ...
        local_iomsg_buildError(cpf, ERR_CNXN_FAILURE, 1, CMERR_BAD_TRANSPORT);
        eips_cnxn_errorFwdOpen(temp_cnxn,cpf);
        return;
    }
    ...
    if(((eips_cpf_getAsmSize(cnxnpt_t2o)*4)+header_size) != (temp_cnxn->T2O_connection_parameters & 0x01FF))
#elif EIPS_USEROBJ_ASM_ELEMENT_SIZE == 2    /* WORDS */
    /* T->O Connection = 2 + ASM size * 2 */
    if(((eips_cpf_getAsmSize(cnxnpt_t2o)*2)+header_size) != (temp_cnxn->T2O_connection_parameters & 0x01FF))
#else                                       /* BYTES */
    /* T->O Connection = 2 + ASM size * 1 */
    if(((eips_cpf_getAsmSize(cnxnpt_t2o)*1)+header_size) != (temp_cnxn->T2O_connection_parameters & 0x01FF))
#endif
    {
        /* Error: "Invalid Connection Size" */
        local_iomsg_buildError(cpf, ERR_CNXN_FAILURE, 1, CMERR_BAD_CONN_SIZE);
        eips_cnxn_errorFwdOpen(temp_cnxn,cpf);
        return;
    }
    ...
    if(((eips_cpf_getAsmSize(cnxnpt_o2t)*4)+header_size) != (temp_cnxn->O2T_connection_parameters & 0x01FF))
#elif EIPS_USEROBJ_ASM_ELEMENT_SIZE == 2    /* WORDS */
    /* O->T Connection = 6 + ASM size * 2 */
    if(((eips_cpf_getAsmSize(cnxnpt_o2t)*2)+header_size) != (temp_cnxn->O2T_connection_parameters & 0x01FF))
#else                                       /* BYTES */
    /* O->T Connection = 6 + ASM size * 1 */
    if(((eips_cpf_getAsmSize(cnxnpt_o2t)*1)+header_size) != (temp_cnxn->O2T_connection_parameters & 0x01FF))
#endif
    {
        /* Error: "Invalid Connection Size" */
        local_iomsg_buildError(cpf, ERR_CNXN_FAILURE, 1, CMERR_BAD_CONN_SIZE);
    ...
    switch(io_type)
    {
        ...
        case EIPS_IOTYPE_INPUTONLY:
            ...
            if((owning_iomsg_struct_ptr != NULL) && (io_type == EIPS_IOTYPE_EXOWNER))
            {
                ...
                local_iomsg_buildError(cpf, ERR_CNXN_FAILURE, 1, CMERR_OWNER_CONFLICT);
                eips_cnxn_errorFwdOpen(temp_cnxn,cpf);
                return;
            }
            ...
            if(iomsg_struct_ptr == NULL)
            {
                ...
                local_iomsg_buildError(cpf, ERR_CNXN_FAILURE, 1, CMERR_NO_CM_RESOURCES);
                eips_cnxn_errorFwdOpen(temp_cnxn,cpf);
                return;
            }
                ...
                iomsg_struct_ptr->CnxnStruct.T2O_API == 0)
            {
                ...
                local_iomsg_buildError(cpf, ERR_CNXN_FAILURE, 1, CMERR_BAD_RPI);
                eips_cnxn_errorFwdOpen(temp_cnxn,cpf);
                return;
            }
            ...
            iomsg_struct_ptr->O2T_Size   = eips_cpf_getAsmSize(iomsg_struct_ptr->O2T_InstID);
            if(iomsg_struct_ptr->O2T_Size > 0)
            {
	            ...
	            if(iomsg_struct_ptr->O2T_DataPtr == NULL)
	            {
                    ...
                    local_iomsg_buildError(cpf, ERR_CNXN_FAILURE, 1, CMERR_NO_SEC_RESOURCE);
                    eips_cnxn_errorFwdOpen(temp_cnxn,cpf);
	                return;
	            }
            }
            ...
            /* try to get the pointer (if needed) */
            iomsg_struct_ptr->T2O_Size   = eips_cpf_getAsmSize(iomsg_struct_ptr->T2O_InstID);
            ...
            temp16 = (uint16)(iomsg_struct_ptr->CnxnStruct.T2O_connection_parameters & NTWKCNXNPRM_CNXNTYP_ANDVAL);
        ...
        case EIPS_IOTYPE_LISTENONLY:
            ...
            {                        
                ...
                local_iomsg_buildError(cpf, ERR_CNXN_FAILURE, 1, CMERR_CONN_REQ_FAILS);
                eips_cnxn_errorFwdOpen(temp_cnxn,cpf);
                return;
            }
...
uint8 eips_iomsg_close (EIPS_CONNECTION_DATA_STRUCT *temp_cnxn)
{
    ...
    for(i=0; i<EIPS_USER_MAX_NUM_IO_CNXNS; i++)
    {
	        ...
	        (eips_stc_iocnxns[i].CnxnStruct.originator_serial_number == temp_cnxn->originator_serial_number))
        {
            ...
            local_iomsg_KickStartMulticast(mult_addr);
            return(TRUE);
        }
    }
...
{
    EIPS_CPF_ITEMSTRUCT Items[EIPS_CPF_MAX_ITEM_CNT];
...
/*            eips_user_dbprint2("UDPIO IP Doesn't Match Ours (%08lx %08lx)\r\n",eips_stc_iocnxns[0].O2T_IPAddr, ipaddr); */
            return(SUCCESS);
        }

        /* validate the data is new */
        if(SEQ32_GT(eips_stc_iocnxns[iomsg_ix].O2T_SeqNum,PassedAddrSeqNum))
        {
            /* this message isn't new */
/*            eips_user_dbprint0("UDPIO Old Data\r\n"); */
	        return(SUCCESS);
        }
...
{
    uint8  prod_buf [EIPS_USER_IOBUF_SIZ];
    ...
    {
        /* Store the Data Item - Length */
#if EIPS_USEROBJ_ASM_ELEMENT_SIZE == 4      /* DWORDS */
        rta_PutLitEndian16((uint16)((iomsg->T2O_Size*4)+2), prod_buf+prod_buf_siz);
#elif EIPS_USEROBJ_ASM_ELEMENT_SIZE == 2    /* WORDS */
        rta_PutLitEndian16((uint16)((iomsg->T2O_Size*2)+2), prod_buf+prod_buf_siz);
#else                                       /* BYTES */
        rta_PutLitEndian16((uint16)((iomsg->T2O_Size)+2), prod_buf+prod_buf_siz);
#endif
    ...
    else
    {
        /* Store the Data Item - Length */
#if EIPS_USEROBJ_ASM_ELEMENT_SIZE == 4      /* DWORDS */
        rta_PutLitEndian16((uint16)(iomsg->T2O_Size*4), prod_buf+prod_buf_siz);
#elif EIPS_USEROBJ_ASM_ELEMENT_SIZE == 2    /* WORDS */
        rta_PutLitEndian16((uint16)(iomsg->T2O_Size*2), prod_buf+prod_buf_siz);
#else                                       /* BYTES */
        rta_PutLitEndian16((uint16)(iomsg->T2O_Size), prod_buf+prod_buf_siz);
#endif
...
static EIPS_IOCNXN_STRUCT * local_iomsg_getOwningConnection (uint16 t2o_cnxnid)
{
    ...
    for(i=0; i<EIPS_USER_MAX_NUM_IO_CNXNS; i++)
    {
        ...
        if ((((uint16)(eips_stc_iocnxns[i].CnxnStruct.T2O_connection_parameters & NTWKCNXNPRM_CNXNTYP_ANDVAL)) 
...
======================================================================= */
static void local_iomsg_KickStartMulticast (uint32 multi_addr)
{    
    uint16 i;
