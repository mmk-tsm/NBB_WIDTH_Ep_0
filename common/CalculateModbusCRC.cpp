...
// protocol. - This function uses a table for the calculation and is much
// faster than the old shift approach.  - Code adapted from internet routine.
// 
// Entry:
...
// M.McKiernan						20-12-2005
// changed int's to WORD (int on Coldfire is 4 bytes).

///////////////////////////////////////////////////////////

#include "CalculateModbusCRC.h"
#include <basictypes.h>
	/* Here's the lookup table for the byte-at-a-time routine. */

const WORD	g_nCRCTable0[] =
{
...
//	ptr = (char*)&nChecksum;
	...
	for( i = 0; i < count; i++ )
	{
			...
			ch = *(buffer + i);			// get data char

			acc0 = g_nCRCTable0[ch ^ (unsigned char) acc0] ^ (unsigned char)(acc0 >> 8);

				/* Nibble-at-a-time processing. Note that because this is
					using the reflected algorithm, the lo-nibble is pro-
					cessed first. */

//			acc1 = g_nCRCTable1[(ch ^ acc1) & 15] ^ (acc1 >> 4);
//			acc1 = g_nCRCTable1[((ch >> 4) ^ acc1) & 15] ^ (acc1 >> 4);
	}
