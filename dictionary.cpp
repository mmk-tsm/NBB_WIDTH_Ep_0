...
void dictionary_dump(dictionary * d, F_FILE * out)
{
	int		i ;
	char	cBuf[MAXVALSZ];
//	sprintf(sval, "%d", val);
//		sprintf(cBuf, "Writing Dictionary to File\n");
//		f_write( cBuf, strlen(cBuf), 1, out);

	if (d==NULL || out==NULL) return ;
//      f_write(buffer,strlen(buffer),1,fp);
//		f_write( cBuf, strlen(cBuf), 1, out);

	if (d->n<1) {
		sprintf(cBuf, "empty dictionary\n");
//      f_write(buffer,strlen(buffer),1,fp);
		f_write( cBuf, strlen(cBuf), 1, out);
		return ;
	}
...
int TestDictionary1( void )
{
   iprintf("TestDictionary...empty \n");
   return(0);
}

int TestDictionaryy( void )
{
	...
	for (i=0 ; i<NVALS ; i++)
   {
     ...
     F_FILE* pFile = f_open("DictFile.txt", "w");
    if( pFile )
    {
       iprintf("\n DictFile opened\n");
     dictionary_dump(d, pFile );
      iprintf("\n DictFile created\n");
      f_close( pFile );
    }
...
int CreateConfigDictionary( void )
{

	dictionary	*	d ;
	char	*	val ;
	double ffWtConstant = 3.7654;
	float fBatchWt = 2.5;
	long nTareCts = 23199;
//	int			i ;
	char		cval[90] ;
...
//		sprintf(cval, "%04d", i);
		dictionary_set( d, "[Hopper]", NULL );

	     sprintf(cval, "%6f", ffWtConstant );
      dictionary_set( d, "Constant", cval );

	     sprintf(cval, "%ld", nTareCts );
      dictionary_set( d, "Tare Value", cval );

      dictionary_set( d, "Double Dump", "YES" );

      sprintf(cval, "%2f", fBatchWt );

      dictionary_set( d, "Batch Weight", cval );
...
*/
     ...
     F_FILE* pFile = f_open("Config.ini", "w");
    if( pFile )
    {
       iprintf("\n Config.ini opened\n");
     dictionary_dump(d, pFile );
    ...
    if( pFile )
    {
      ...
      f_close( pFile );
  	    ...
  	    d = iniparser_load("Config.ini");

      iprintf("\n iniparser_loaded\n");
 		long lTareCounts = iniparser_getint(d, "Hopper:Tare Value", 0);
		printf("\n (a). Tare Counts = %ld \n", lTareCounts);



	printf("getting %d values...\n", NVALS);
		val = dictionary_get(d, "Hopper:Double Dump", DICT_INVALID_KEY);
		if (val==DICT_INVALID_KEY)
      {
			printf("cannot get value for key [%s]\n", cval);
      }
      ...
      {
         printf("\nkey = %s, value = %s\n", "Hopper:Double Dump", val );
		}
		lTareCounts = dictionary_getint(d, "Hopper:Tare Value", 0);
		printf("\n Tare Counts = %ld \n", lTareCounts);
      lTareCounts = iniparser_getint(  d,  "Hopper:Tare Value", 0);
 		printf("\n Tare Counts 2 = %ld \n", lTareCounts);

 		val = iniparser_getstring(d, "Hopper:Double Dump", NULL );
      printf("\nkeyy = %s, value = %s\n", "Hopper:Double Dump", val );
